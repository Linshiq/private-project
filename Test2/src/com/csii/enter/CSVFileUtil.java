...
public class CSVFileUtil {
	...
	public static void ziduandingyi() {
		try {
			CSVFileUtil s = new CSVFileUtil("F:\\负债表字段定义.csv");
			// System.out.println( s.readLine());
			int j = 0;
			while (j < 54) {
				StringBuffer sb = new StringBuffer();
				sb.append("insert into apb_dictionary (ch_field, en_field, ide_id, remark) ").append("\n");
				for (int i = 0; i < 10; i++) {
					...
					str.replaceAll(",", "");
					if (i == 0) {
						String lenStrOne = str.substring(0, str.indexOf(",")) + "',";
						if (lenStrOne.length() < 18) {
							for (int k = lenStrOne.length(); k <= 19; k++)
								lenStrOne = lenStrOne + " ";
						}
						String lenStrTwo = str.substring(str.indexOf(",") + 1, str.lastIndexOf(",")) + "',";
						if (lenStrTwo.length() < 46) {
							for (int k = lenStrTwo.length(); k <= 44; k++)
								lenStrTwo = lenStrTwo + " ";
						}
						String lenStrT = str.substring(str.lastIndexOf(",") + 1) + "',";
						if (lenStrT.length() < 31) {
							for (int k = lenStrT.length(); k <= 31; k++)
								lenStrT = lenStrT + " ";
						}
						sb.append("      select    ").append("'" + lenStrOne);
						sb.append("'").append(lenStrTwo);
						sb.append("'").append(lenStrT);
						...
						sb.append("union select    ").append("'" + lenStrOne);
						sb.append("'").append(lenStrTwo);
						sb.append("'").append(lenStrT);
						sb.append("''           					   			   from dual").append("\n");
					}
	...
	public static void tabledingyi() {

		try {
			CSVFileUtil s = new CSVFileUtil("11F:\\负债表字段定义.csv");
			System.out.println();
			...
			sb.append(
					"insert into ctp_trxn_mapping (CT_TRXN_CODE, TRXN_CODE_NAME, BACKGD_TRXN_CODE, DATA_CREATE_USER, DATA_CREATE_TIME, DATA_UPDATE_USER, DATA_UPDATE_TIME, DATA_VERSION)");

			while ((nextLine = s.readLine()) != null) {

				String[] strA = nextLine.split(",");
				...
				sb.append(" null, null, null, null, '1' from dual");
